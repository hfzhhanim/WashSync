...
class _DryerPageState extends State<DryerPage> {
    ...
    _cleanupTimer = Timer.periodic(const Duration(seconds: 10), (timer) async {
      ...
      for (var doc in snapshot.docs) {
        ...
        if (startTimeString != null) {
          ...
          // If the clock hits the start time (or passes it) and user hasn't confirmed
  ...
  // Design/UI methods preserved exactly as per your requirement
  ...
  Future<void> _handlePayAndStart(Booking booking) async {
    ...
    final bool? success = await Navigator.push(context, MaterialPageRoute(builder: (_) => PaymentScreen(machineType: "Dryer", machineNo: machineNumber)));
    if (success == true) {
      ...
      batch.update(_firestore.collection('dryers').doc(booking.machineId), {
    ...
    if (snapshot.docs.isEmpty) {
      for (int i = 1; i <= 5; i++) {
        await dryerCollection.doc(i.toString()).set({
          'name': 'Dryer $i', 'endTime': '', 'currentRemark': '', 'status': 'Available', 'maintenanceStart': null, 'maintenanceUntil': null,
        });
      }
    }
  }
            ...
            return StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, bookingSnapshot) {
                ...
                final myActiveBookings = allBookings.where((b) {
                  ...
                  // Show if cycle running OR if slot hasn't expired yet
  ...
  Widget _buildDryerCard(DryingMachine machine, List<Booking> allBookings) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          Icon(isUnderMaintenance ? Icons.construction : Icons.dry, color: isUnderMaintenance ? Colors.red : (isInUse ? Colors.blue : Colors.purple), size: 28),
      ...
      child: Column(children: [
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _selectedSlot == null ? null : () {
          widget.onSave(Booking(
            ...
            startTime: _selectedSlot!, endTime: _selectedSlot!.add(Duration(minutes: _slotDuration)), type: 'Dryer',
          ));
